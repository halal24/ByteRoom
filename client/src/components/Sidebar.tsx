...
const FullLogo = () => (
  <svg viewBox="0 0 400 150" xmlns="http://www.w3.org/2000/svg" className="w-full h-auto shrink-0">
    <defs>
...
const Sidebar = () => {
  ...
  return (
    <>
      {/* Inject CSS-only hover styles — zero JS state, zero flicker */}
      <style>{`
        .br-sidebar {
          width: 68px;
        ...
        .br-sidebar:hover .br-label {
          opacity: 1;
          max-width: 160px;
        }
        .br-sidebar .br-logo-icon {
          opacity: 1;
          transform: scale(1);
          transition: opacity 200ms ease, transform 280ms ease;
          position: absolute;
        }
        .br-sidebar:hover .br-logo-icon {
          opacity: 0;
          transform: scale(0.7);
        }
        .br-sidebar .br-logo-full {
          opacity: 0;
          transform: scale(0.92);
          transition: opacity 220ms ease 60ms, transform 280ms ease;
        }
        .br-sidebar:hover .br-logo-full {
          opacity: 1;
          transform: scale(1);
        }
      `}</style>

      <div className="br-sidebar relative flex flex-col border-r border-white/10 bg-dark-800/50 min-h-screen">
        {/* Logo Area */}
        <div className="relative h-20 flex items-center justify-center px-2">
          <div className="br-logo-icon">
            <BracketIcon />
          </div>
          <div className="br-logo-full w-full px-3">
            <FullLogo />
          </div>
        </div>
        ...
        <nav className="flex-1 px-2 space-y-1 mt-2">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                {item.icon}
                <span className="br-label text-sm font-medium">{item.label}</span>
              </Link>
            );
          })}
        </nav>
        ...
        <div className="border-t border-white/10 p-2 mt-auto">
          ...
          >
            ...
            <span className="br-label text-sm font-medium">Sign Out</span>
          </button>
        </div>
      </div>
    </>
...
export const useSidebarState = () => ({ collapsed: false, toggle: () => {} });
